...
Do NOT manually modify Assets/base2edit.js. All Javascript modifications should be applied to the Typescript files in frontend directory (or scripts). It is OK to modify the CSS file(s) within this project directly.
...
The `run-tests` script lives in **this extension’s root directory**: the folder that contains `SwarmUI-Base2Edit.csproj`, `SwarmUI-Base2Edit.Tests.sln`, and `run-tests`. It is **not** at the main SwarmUI repository root and not inside `src/`, `Tests/`, or `frontend/` unless that is already the extension root.
...
2. **Call it by path from the SwarmUI repo root**:
   ...
   `src/Extensions/SwarmUI-Base2Edit/run-tests`
